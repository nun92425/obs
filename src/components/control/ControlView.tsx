...
export function ControlView({ streams, setStreams }: { streams: Map<string, MediaStream>; setStreams: React.Dispatch<React.SetStateAction<Map<string, MediaStream>>> }) {
  ...
  const renderPreviewProps = () => {
    ...
    if (preview.type === 'standby') {
      ...
      return (
        <div className="space-y-2 bg-zinc-900 p-3 rounded">
          ...
          <input value={(preview as any).text} onChange={(e) => updateSource(preview.id, { text: e.target.value } as any)} placeholder="メイン文（空にすると文字なしで動画のみ）" className="w-full bg-zinc-800 rounded px-2 py-1 text-sm" />
          <input value={(preview as any).subText || ''} onChange={(e) => updateSource(preview.id, { subText: e.target.value } as any)} placeholder="サブ文" className="w-full bg-zinc-800 rounded px-2 py-1 text-sm" />
          <div className="flex gap-2 items-center flex-wrap">
            ...
            <input type="file" accept="video/*" onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; const url = URL.createObjectURL(f); updateSource(preview.id, { url } as any) }} className="text-xs" />
            {(preview as any).url && <button onClick={() => updateSource(preview.id, { url: undefined } as any)} className="text-xs px-2 py-1 bg-zinc-800 rounded hover:bg-red-900">動画クリア</button>}
          </div>
          <div className="flex gap-3 items-center bg-zinc-800/50 p-2 rounded">
            <label className="flex items-center gap-1 text-xs">
              <input type="checkbox" checked={(preview as any).enableOverlay ?? true} onChange={(e) => updateSource(preview.id, { enableOverlay: e.target.checked } as any)} />
              暗幕
            </label>
            <label className="flex items-center gap-1 text-xs flex-1">
              濃さ
              <input type="range" min={0} max={0.8} step={0.05} value={(preview as any).overlayOpacity ?? 0.35} onChange={(e) => updateSource(preview.id, { overlayOpacity: parseFloat(e.target.value) } as any)} disabled={!((preview as any).enableOverlay ?? true)} className="flex-1" />
    ...
    if (preview.type === 'bgm') {
      return (
        <div className="space-y-2 bg-zinc-900 p-3 rounded">
          <div className="text-sm font-bold">BGM設定</div>
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={(preview as any).loop} onChange={(e) => updateSource(preview.id, { loop: e.target.checked } as any)} /> ループ
          </label>
          <label className="flex items-center gap-2 text-sm">
      ...
      return (
        <div className="space-y-2 bg-zinc-900 p-3 rounded">
          <div className="flex gap-2">
            <button onClick={() => setEditingSlideId(preview.id)} className="px-3 py-1 bg-sky-600 rounded text-sm">エディタを開く</button>
            <button onClick={() => { const idx = (preview as any).currentIndex; updateSource(preview.id, { currentIndex: Math.max(0, idx - 1) } as any) }} className="px-2 py-1 bg-zinc-800 rounded text-sm">◀ 前</button>
            <span className="text-sm py-1">{(preview as any).currentIndex + 1} / {(preview as any).slides.length}</span>
            <button onClick={() => { const idx = (preview as any).currentIndex; updateSource(preview.id, { currentIndex: Math.min((preview as any).slides.length - 1, idx + 1) } as any) }} className="px-2 py-1 bg-zinc-800 rounded text-sm">次 ▶</button>
          </div>
        </div>
      )
    }
    if (preview.type === 'screen') {
      return (
        <div className="bg-zinc-900 p-3 rounded space-y-2">
          ...
          <p className="text-xs text-zinc-400">Canvaでスライドを再生しながら「画面共有開始」でそのまま投影できます。</p>
          <button onClick={() => handleScreenShare(preview.id)} className="px-3 py-1 bg-emerald-600 rounded text-sm">画面共有開始</button>
        </div>
      )
    }
    ...
    <div className="min-h-screen bg-[#0f0f12] text-zinc-100 flex flex-col">
      <header className="flex items-center justify-between px-4 py-2 bg-zinc-900 border-b border-zinc-800 sticky top-0 z-20">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-zinc-500 hidden md:inline">Space=TAKE / B=BLACK / ?=ヘルプ</span>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setHelpOpen(true)} className="px-3 py-1 bg-zinc-800 hover:bg-zinc-700 rounded text-sm">？</button>
          <button onClick={openOutput} className="px-3 py-1 bg-zinc-800 hover:bg-zinc-700 rounded text-sm">出力ウィンドウを開く</button>
          <button onClick={openFullscreenOutput} className="px-3 py-1 bg-red-600 hover:bg-red-500 rounded text-sm">全画面出力</button>
          <button onClick={exportJson} className="px-3 py-1 bg-zinc-800 rounded text-sm">保存</button>
          <button onClick={() => importRef.current?.click()} className="px-3 py-1 bg-zinc-800 rounded text-sm">読込</button>
      ...
      <div className="p-3 space-y-3 flex-1">
        ...
        <div className="flex flex-wrap gap-2 items-center bg-zinc-900 p-3 rounded">
          <button onClick={handleTake} className="px-6 py-2 bg-red-600 hover:bg-red-500 rounded font-bold">{transition === 'fade' ? 'TAKE (FADE)' : 'CUT'}</button>
          <button onClick={() => (transition === 'fade' ? take() : cut())} className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 rounded text-sm">CUT</button>
          <button onClick={() => setBlack(!isBlack)} className={`px-4 py-2 rounded text-sm ${isBlack ? 'bg-red-600' : 'bg-zinc-800 hover:bg-zinc-700'}`}>BLACK</button>
          ...
          <label className="flex items-center gap-1 text-xs">
            映像
            <input type="range" min={0} max={1} step={0.05} value={mixer.videoVolume} onChange={(e) => setMixer({ videoVolume: parseFloat(e.target.value) })} />
          </label>
          <label className="flex items-center gap-1 text-xs">
        ...
        {slideEditing && (
          <div className="fixed inset-0 bg-black/80 z-50 flex flex-col">
            <div className="flex justify-between p-2 bg-zinc-900">
              ...
              <button onClick={() => setEditingSlideId(null)} className="px-3 py-1 bg-zinc-800 rounded">閉じる</button>
            </div>
